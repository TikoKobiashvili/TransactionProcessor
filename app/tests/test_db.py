...
def test_initialize_db(mock_get_db_connection):
    ...
    expected_commands = [
        ...
        '( id SERIAL PRIMARY KEY, provider_name VARCHAR(255)'
        ' NOT NULL, initial_value NUMERIC NOT NULL );',
        'CREATE TABLE IF NOT EXISTS historical_transactions'
        ' ( id SERIAL PRIMARY KEY, provider_id INTEGER REFERENCES initial_data(id),'
        ' transaction_value NUMERIC NOT NULL );',
        'INSERT INTO initial_data (provider_name, initial_value)'
        " VALUES ('Visa', 1000), ('Mastercard', 2000) ON CONFLICT DO NOTHING;",
        'INSERT INTO historical_transactions (provider_id, transaction_value)'
        ' VALUES (1, 100), (1, 200), (2, -200) ON CONFLICT DO NOTHING;',
    ]
    ...
    assert all(
        command in normalized_actual_commands for command in expected_commands
    ), f'Expected commands: {expected_commands}, but got: {normalized_actual_commands}'
